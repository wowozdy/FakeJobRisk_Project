...
def run_model(model_name, pipeline, X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    cm_path = FIG_DIR / f"{model_name}_test_confusion_matrix.png"
    ...
    pred_df.to_csv(TABLE_DIR / f"{model_name}_test_predictions.csv", index=False, encoding="utf-8-sig")
...
def main():
    ...
    # Full feature preprocessors
    full_preprocessor = build_full_preprocessor()

    # Structured-only preprocessor for random forest
    ...
    results_df.to_csv(TABLE_DIR / "baseline_metrics.csv", index=False, encoding="utf-8-sig")
    print("\n========== Summary ==========")
    print(results_df.round(4).to_string(index=False))
    print(f"\nSaved metrics to: {TABLE_DIR / 'baseline_metrics.csv'}")
